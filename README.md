...
HTTPS環境ではブラウザの「ホーム画面に追加」からインストールできる。認証と投稿同期にはネットワーク接続が必要。
...
新規ユーザーのオンボーディングは認証方法にかかわらず `/onboarding/profile` から開始し、ユーザー名、モード、グループ作成／参加、猫選択の順でAPIへ保存する。既存プロフィールのあるユーザーは再入力せず、主要画面の `/home` を起点にする。
名前入力画面からログアウトしてログイン画面へ戻れる。初回ガイドの進行状況はユーザー単位で保存し、選択したモードにかかわらずホームから開始する。
名前の登録完了後は、それ以前の画面へ戻る導線を残さない。新規登録中のグループ作成には戻る導線を出さず、グループ一覧から追加する場合だけ一覧へ戻れる。
完了方向の画面遷移は履歴を置き換え、ログイン後に戻る操作で完了済みの入力画面へ戻らない。

- `/onboarding/cat`: 猫選択とプロフィール保存
- `/onboarding/invite`: 招待コード表示・コピー
- `/home`: 現在のグループの投稿空間
- ホームの猫とつぶやきは、ドラッグして表示位置を動かせる
- ホームのつぶやきパネル: 30文字投稿と表情選択
- `/chat`: 現在のグループのチャット閲覧・送信
